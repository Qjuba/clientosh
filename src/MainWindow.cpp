...
void MainWindow::applyTheme()
{
    ...
    const QString qss = fill(QStringLiteral(
        ...
        "  background: {{tableBg}};"
        "}"
        "QTableWidget#dashTable::item { padding: 4px 8px; }"
        "QTableWidget#dashTable::item:selected { background: {{selectedBg}}; color: {{textBright}}; }"
        "QTableWidget#dashTable::item:hover { background: {{hoverBg}}; }"
        "QHeaderView::section {"
        "  background: {{headerBg}};"
        "  color: {{textMuted}};"
        "  border: none;"
