...
ConnectionMode modeFromCommand(const QString& cmd, QString* errorOut)
{
    ...
    if (errorOut) {
        *errorOut = QStringLiteral("unknown command %1 (expected ssh, telnet, or sftp)").arg(cmd);
    }
    return ConnectionMode::Ssh;
}
...
void configureParser(QCommandLineParser& parser)
{
    parser.setApplicationDescription(
        QStringLiteral(
            "clientosh - SSH, SFTP, and Telnet client\n"
            "\n"
            "Quick connect:\n"
            ...
            "\n"
            "Target: host, host:port, user@host, or user@host:port\n"
            "Run without a command to open the dashboard."));

    parser.addPositionalArgument(
        QStringLiteral("command"),
        QStringLiteral("Connection type: ssh, telnet, or sftp."));
    parser.addPositionalArgument(
...
Request parse(const QCommandLineParser& parser)
{
    ...
    if (!profile.isTelnet() && profile.user.isEmpty()) {
    ...
    if (req.openSftpWithSsh && profile.isTelnet()) {
